...
系统 MUST 在当前受管视频处于全屏播放上下文时支持键盘播放控制：按空格键切换播放/暂停，按左方向键后退 10 秒，按右方向键前进 10 秒。

#### Scenario: 空格键切换播放状态
- **WHEN** 当前受管视频处于全屏播放上下文，且焦点不在可编辑区域中，用户按下空格键
- **THEN** 系统 MUST 在播放与暂停之间切换当前视频状态

#### Scenario: 左方向键后退 10 秒
- **WHEN** 当前受管视频处于全屏播放上下文，且焦点不在可编辑区域中，用户按下左方向键
- **THEN** 系统 MUST 将当前视频播放位置回退 10 秒，并且不得小于 0 秒

#### Scenario: 右方向键前进 10 秒
- **WHEN** 当前受管视频处于全屏播放上下文，且焦点不在可编辑区域中，用户按下右方向键
- **THEN** 系统 MUST 将当前视频播放位置前进 10 秒，并且不得超过视频可播放结束位置
