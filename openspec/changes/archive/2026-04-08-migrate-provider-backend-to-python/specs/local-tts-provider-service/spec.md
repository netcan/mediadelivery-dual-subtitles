...
系统 MUST 在本机提供与扩展当前契约兼容的 HTTP 接口，以接收中文配音任务并返回任务状态或结果；推荐实现 MUST 迁移为 Python Provider 服务，而不是仅依赖 Node.js 版本。同时，服务 MUST 允许前端查询 Provider 能力，并据此展示可选音色。
...
#### Scenario: 查询可选音色
- **WHEN** 扩展需要展示当前 Provider 的可选音色
- **THEN** 服务 MUST 返回可选音色列表与默认音色，而不要求前端硬编码模型选项

#### Scenario: 使用自定义 Provider 地址
- **WHEN** 扩展被配置为连接某个自定义 IP / 端口 / Base URL 的 Provider
- **THEN** 只要该地址实现兼容契约，扩展 MUST 能正常发起任务并查询状态

### Requirement: 本地 Provider 服务必须输出扩展可消费的结果结构
系统 MUST 在任务完成时返回至少包含音频 URL 的结果结构，并允许附带字幕 URL 与最小元数据；无论后端语言为 Python 还是 Node，结果结构 MUST 保持与扩展现有消费逻辑兼容。
...
系统 MUST 在本地模型不可用、推理失败或输入无效时返回可解释的错误状态；推荐实现 MUST 明确区分 Python 运行环境问题、模型初始化失败和任务处理失败。
