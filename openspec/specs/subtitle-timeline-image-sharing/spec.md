# subtitle-timeline-image-sharing Specification

## Purpose
定义宿主页悬浮字幕时间轴中的多选与双语分享图片能力，支持从已存在的字幕内容中快速摘录、复制和分享。
...
系统 MUST 能根据当前选中的字幕条目生成一张可复制的双语分享图片，并在图片中保留时间戳、英文字幕和中文字幕。

#### Scenario: Generate image from selected items
- **WHEN** 用户已选择至少一条字幕并触发分享图片操作
- **THEN** 系统 MUST 生成包含所选字幕条目的双语图片，并允许用户复制 PNG 图片到剪贴板
